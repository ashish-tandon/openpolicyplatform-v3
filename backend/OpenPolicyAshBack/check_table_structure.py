...
def check_table_structure():
    """Check the actual structure of the jurisdictions table"""
    engine = create_engine(get_database_url())
    
    print("🔍 Checking database tables...")
    
    with engine.connect() as conn:
        # Check all tables from all schemas
        ...
        # Check specifically for jurisdictions
        result = conn.execute(text("""
            SELECT schemaname, tablename 
            FROM pg_tables 
        ...
        if jurisdictions:
            ...
            print("\n❌ Jurisdictions table not found in any schema!")
